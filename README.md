...
- **Docker** is available for RPI since 2016, see the announcement at this  [post](https://www.raspberrypi.org/blog/docker-comes-to-raspberry-pi/) from Matt Richardson, director of Raspberry Pi Foundation North America. This among other tutorials are available for Docker setup at RPI. 
...
After the first run you should be patient to access the `Thing` device, since a lot of stuff is loaded and prepared by .NET Core and ASP.NET platforms. Please see below access to both  `home-ui` and `home-web`. 
