...
Please note that `home-web` is using `http` protocol to show swagger UI. The other window from `home-ui` shows `https` protocol being used in the default "About" page. The self-signed certificate generates a "Not secure" alert from the browser that may be fixed installing a true certificate.
