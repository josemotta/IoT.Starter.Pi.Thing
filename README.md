...
Please note that `home-web` is using `http` protocol to show swagger UI. The other window from `home-ui` shows `https` protocol being used in the default "About" page. As you can see, the self-signed certificate generates a "Not secure" alert from the browser. This should be fixed  with a true certificate generated by proper `key.pem` and `cert.pem` files.
